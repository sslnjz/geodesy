...
| Phase 5 | Ellipsoidal Coordinates And Cartesian | Phase 5B completed with known baseline failures | `docs/migration/reports/phase-05b-cartesian-report.md` | Cartesian/ECEF to LatLon reverse conversion hardened for invalid Cartesian coordinates, invalid ellipsoid inputs, polar-axis handling, non-WGS84 round-trip, and non-finite height rejection. Focused ellipsoidal test passed. Debug configure/build succeeded after allowing Windows SDK metadata access. Required raw CTest still needs `-C Debug`; full Debug CTest still has the existing Vincenty encoding failure. |
| Phase 6 | Datum And Helmert 7-Parameter Transforms | Completed with known baseline failures | `docs/migration/reports/phase-06-datum-report.md` | Datum conversion now validates datum ellipsoids explicitly, preserves WGS84 identity, handles WGS84<->OSGB36 and non-WGS84 chained Helmert paths without relying on zero-valued transform truthiness, and rejects unrecognised datum paths. Focused datum test passed. Debug configure/build succeeded after allowing Windows SDK metadata access. Required raw CTest still needs `-C Debug`; full Debug CTest still has the existing Vincenty encoding failure. |
| Phase 7 | Vincenty Geodesics | Completed | `docs/migration/reports/phase-07b-vincenty-direct-report.md` | Phase 7A inverse and Phase 7B direct solution are complete. Direct now validates non-finite distance/bearing, keeps zero-distance and height rejection deterministic, uses named convergence tolerance/iteration limits, and normalizes anti-meridian destinations. Focused Vincenty test passed. Debug configure/build succeeded after allowing Windows SDK metadata access. Required raw CTest still needs `-C Debug`; full Debug CTest passed. |
| Phase 8 | UTM | Phase 8A completed | `docs/migration/reports/phase-08a-utm-type-report.md` | UTM value type validation, parsing, formatting, convergence/scale accessors, and focused UTM tests are complete. Latitude/longitude forward and reverse projection hardening remains open for the next Phase 8 slice. |
